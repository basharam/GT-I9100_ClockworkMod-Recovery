...
{
	TJ4FS(J4FS_TRACE_LOCK, ("j4fs locking %p\n", current));
	down(&device_info.grossLock);
	TJ4FS(J4FS_TRACE_LOCK, ("j4fs locked %p\n", current));
}

void j4fs_GrossUnlock(void)
{
	TJ4FS(J4FS_TRACE_LOCK, ("j4fs unlocking %p\n", current));
	up(&device_info.grossLock);
}

int j4fs_readpage(struct file *f, struct page *page)
{
	TJ4FS(J4FS_TRACE_FS_READ,("%s %d\n",__FUNCTION__,__LINE__));
...
int j4fs_writepage(struct page *page, struct writeback_control *wbc)
{
	...
	if(j4fs_panic==1) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: j4fs panic\n",__FUNCTION__,__LINE__));
		return -ENOSPC;
	}

	TJ4FS(J4FS_TRACE_FS,("%s %d\n",__FUNCTION__,__LINE__));
				...
				struct page **pagep, void **fsdata)
{
	...
	TJ4FS(J4FS_TRACE_FS, ("start j4fs_write_begin\n"));

	if(to>PAGE_CACHE_SIZE) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: page size overflow(pos,index,offset,len,to)=(%d,%d,%d,%lu,%lu)\n",__FUNCTION__,__LINE__,pos,index,offset,len,to));
	...
	*pagep = pg;
	...
	/* Happy path return */
	TJ4FS(J4FS_TRACE_FS, ("end j4fs_write_begin - ok\n"));

	return 0;

out:
	TJ4FS(J4FS_TRACE_FS, ("end j4fs_write_begin fail returning %d\n", ret));
				...
				unsigned offset, unsigned to)
{
	...
	if(to>PAGE_CACHE_SIZE) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: page size overflow(offset,to)=(%d,%d)\n",__FUNCTION__,__LINE__,offset,to));
		j4fs_panic("page size overflow");
		return -ENOSPC;
	}
				...
				struct page *pg, void *fsdadata)
{
	...
	if(offset_into_page+copied > PAGE_CACHE_SIZE) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: page size overflow(offset_into_page,copied)=(%d,%d)\n",__FUNCTION__,__LINE__,offset_into_page, copied));
		j4fs_panic("page size overflow");
		return -ENOSPC;
	}
	...
	if (ret != copied) {
		TJ4FS(J4FS_TRACE_ALWAYS, ("j4fs_write_end not same size ret %d  copied %d\n", ret, copied));
		SetPageError(pg);
...
int j4fs_commit_write(struct file *f, struct page *pg, unsigned offset, unsigned to)
{
	...
	if(offset+nBytes > PAGE_CACHE_SIZE) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: page size overflow(offset,nBytes)=(%d,%d)\n",__FUNCTION__,__LINE__, offset, nBytes));
		j4fs_panic("page size overflow");
		return -ENOSPC;
	}
	...
	TJ4FS(J4FS_TRACE_FS, ("j4fs_commit_write: (addr,pos,nBytes)=(0x%x, 0x%x, 0x%x)\n", saddr, spos, nBytes));

	nWritten = j4fs_file_write(f, addr, nBytes, &pos);

	if (nWritten != nBytes) {
		TJ4FS(J4FS_TRACE_ALWAYS, ("j4fs_commit_write: (nWritten,nBytes)=(0x%x 0x%x)\n", nWritten, nBytes));
		SetPageError(pg);
		ClearPageUptodate(pg);
	} else {
		SetPageUptodate(pg);
...
int j4fs_file_write(struct file *f, const char *buf, size_t n, loff_t *pos)
{
	...
	TJ4FS(J4FS_TRACE_FS,("j4fs_file_write: %zu bytes to ino %ld at %d\n", n, inode->i_ino, ipos));
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error(nWritten=0x%x)\n",__FUNCTION__,__LINE__,nWritten));
		j4fs_GrossUnlock();
		return -ENOSPC;
	}
...
struct j4fs_inode *j4fs_get_inode(struct super_block *sb, ino_t ino)
{
	...
	while(cur_link!=0xffffffff)
	{
		...
		if (nErr != 0) {
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: error(nErr=0x%x)\n",__FUNCTION__,__LINE__,nErr));
	   		goto error1;
		}
...
Einval:
	TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: error(bad inode number: %lu)\n",__FUNCTION__,__LINE__,(unsigned long) ino));
	kfree(buf);
...
struct inode *j4fs_new_inode(struct inode *dir, struct dentry *dentry, int mode)
{
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: partition size overflow(new_object_offset=0x%08x, j4fs_end=0x%08x)\n",__FUNCTION__,__LINE__,new_object_offset,device_info.j4fs_end));
		goto error1;
	}
...
int j4fs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (ret) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error(0x%08x)\n",__FUNCTION__,__LINE__,ret));
		return -EINVAL;
	}
...
// J4FS for moviNAND merged from ROSSI
	...
	TJ4FS(J4FS_TRACE_FS,("%s %d: device_info.j4fs_end=0x%08x, device_info.j4fs_device_end=0x%08x\n",__FUNCTION__,__LINE__,device_info.j4fs_end,device_info.j4fs_device_end));
	...
	TJ4FS(J4FS_TRACE_FS,("%s %d: j4fs_next_sequence=0x%08x, j4fs_transaction_next_offset=0x%08x\n",__FUNCTION__,__LINE__,j4fs_next_sequence,j4fs_transaction_next_offset));
#endif
