...
#define TJ4FS(mask, p) do { if ((mask) & (j4fs_traceMask | J4FS_TRACE_ALWAYS)) TOUT(p); } while (0)
...
do {							\
	TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: "str"\n",__FUNCTION__,__LINE__));	\
	fsd_panic();		\
	dump_stack();	\
} while (0)
#else
...
do {		\
	if(offset + J4FS_BASIC_UNIT_SIZE > device_info.j4fs_end) {	\
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: offset overflow(offset=0x%08x, j4fs_end=0x%08x)\n", __FUNCTION__, __LINE__, offset, device_info.j4fs_end));	\
