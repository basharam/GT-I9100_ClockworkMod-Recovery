...
int fsd_read(j4fs_ctrl *ctl)
{
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error! j4fs_rw_start is invalid(j4fs_rw_start=0x%08x, j4fs_end=0x%08x, ro_j4fs_header_count=0x%08x)\n",
	...
	while(offset!=0xffffffff)
	{
		...
		if (error(ret)) {
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error(nErr=0x%08x)\n",__FUNCTION__,__LINE__,ret));
			goto error1;
		}
...
got_header:
	...
	if(matching_offset!=0xffffffff)
	{
		...
		{
			TJ4FS(J4FS_TRACE_FSD,("%s %d: (offset,count,len)=(0x%08x,%d,%d)\n",__FUNCTION__,__LINE__,matching_offset,count,len));
...
int fsd_write(j4fs_ctrl *ctl)
{
	...
	TJ4FS(J4FS_TRACE_FSD,("%s %d: (ino,index)=(%d,0x%08x)\n",__FUNCTION__,__LINE__,ctl->id,ctl->index));
	...
	if(ctl->index < 0)
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error! ctl->index is invalid(ctl->index=0x%08x)\n",
	...
	if(ctl->count==0)
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: count is zero\n",__FUNCTION__, __LINE__));
	#ifdef __KERNEL__
		kfree(buf);
	#ifdef J4FS_TRANSACTION_LOGGING
		kfree(transaction);
	...
	if( (matching_latest_offset<j4fs_rw_start) ||(matching_latest_offset>device_info.j4fs_end) ) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: matching_latest_offset is invalid(matching_latest_offset=0x%08x)\n",__FUNCTION__,__LINE__,matching_latest_offset));
	...
	if( (last_object_offset<j4fs_rw_start) ||(last_object_offset>device_info.j4fs_end) ) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: last_object_offset is invalid(last_object_offset=0x%08x)\n",__FUNCTION__,__LINE__,last_object_offset));
	...
	if(is_it_last_object)
	{
		...
		{
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: j4fs don't support file hole(matching_latest_object_length,ctl->index)=(0x%08x,0x%08x)\n",__FUNCTION__,__LINE__,matching_latest_object_length,ctl->index));
		...
		else if(matching_latest_object_length == ctl->index)	// when extend file size(2,3,5,6)
		{
			...
			{
				TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Reclaim is needed(offset,ctl->count,j4fs_end)=(0x%08x,0x%08x,0x%08x)\n",__FUNCTION__,__LINE__,offset,ctl->count,device_info.j4fs_end));
				ret=fsd_reclaim();
			...
			while(ctl->count>=device_info.pagesize)
			{
				...
				TJ4FS(J4FS_TRACE_FSD,("%s %d: (buffer_index,offset,ctl->count)=(0x%08x,%d,%d)\n",__FUNCTION__,__LINE__,buffer_index,offset,ctl->count));
		...
		{
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: we should not come here\n",__FUNCTION__,__LINE__));
	...
	else if(!is_it_last_object)
	{
		...
		else if(matching_latest_object_length >= (ctl->index+ctl->count))	// update not-last object in partition and No extension(7)
		{
			...
			if((offset+ctl->count)>device_info.j4fs_end)
			{
				TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: This case should not happen(offset,ctl->count,j4fs_end)=(0x%08x,0x%08x,0x%08x)\n",__FUNCTION__,__LINE__,offset,ctl->count,device_info.j4fs_end));
		...
		else if(matching_latest_object_length < (ctl->index+ctl->count))	// update not-last object in partition and file size is extended(9,11,12,13)
		{
			...
			{
				TJ4FS(J4FS_TRACE_FSD,("%s %d: (matching_latest_offset,new_header_offset,buffer_index)=(0x%08x,0x%08x,0x%08x)\n",
	...
	TJ4FS(J4FS_TRACE_FSD,("%s %d: write completed(written=%d)\n",__FUNCTION__,__LINE__,buffer_index));
...
int fsd_reclaim()
{
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: MST is not recognized(mst.magic=0x%08x)\n",__FUNCTION__,__LINE__,mst->magic));
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: j4fs is crashed(mst.status=0x%08x)\n",__FUNCTION__,__LINE__,mst->status));
		j4fs_panic("j4fs is crashed");
   		goto error1;
	}

	TJ4FS(J4FS_TRACE_FSD_RECLAIM,("\n%s %d: Reclaim Starts\n",__FUNCTION__,__LINE__));
	fsd_print_meta_data();
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error! rw_start is invalid(j4fs_rw_start=0x%08x, j4fs_end=0x%08x)\n", __FUNCTION__, __LINE__, mst->rw_start, device_info.j4fs_end));
...
update_link:
	...
	if(first_unused_area_offset!=0xffffffff)
	{
		...
		{
			TJ4FS(J4FS_TRACE_FSD_RECLAIM,("%s %d: (first_unused_area_offset=0x%08x,i=0x%08x,j4fs_end=0x%08x)\n",__FUNCTION__,__LINE__,first_unused_area_offset,i,device_info.j4fs_end));
...
reclaim_done:
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: j4fs_rw_start is set to default value(128KB)\n",__FUNCTION__,__LINE__));
...
int fsd_mark_invalid()
{
	...
	{
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error! MST is invalid\n", __FUNCTION__, __LINE__));
	...
	TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("%s %d: index=%d\n",__FUNCTION__,__LINE__,index));
	...
	for(i=0;i<index-2;i++)
	{
		for(j=i+1;j<index-1;j++)
		{
			...
			{
				//TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("%s %d\n",__FUNCTION__,__LINE__));
...
int fsd_read_ro_header(void)
{
	...
	while(offset<j4fs_rw_start)
	{
		...
		if(header->type!=J4FS_FILE_TYPE)
		{
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: ERROR! RO file cannot be interpreted. (offset=%d)\n",__FUNCTION__,__LINE__,offset));
			j4fs_panic("This j4fs_header cannot be interpreted. So this j4fs partition is crashed by some abnormal cause.  This should not happen and should be repaired.");
			goto error1;
		}
		...
		if((header->flags&0x1)!=((header->flags&0x2)>>1))
		{
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: ERROR! RO file was deleted. (offset,ino,filename)=(%d,%d,%s)\n",__FUNCTION__,__LINE__,offset,header->id,header->filename));
			offset=header->link;
			continue;
		}
		...
		{
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: ERROR! Too many ro files\n",__FUNCTION__,__LINE__));
	...
	TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("\n====================== %s %d ================================\n", __FUNCTION__,__LINE__));

	for(i=0;i<ro_j4fs_header_count;i++)
	{
		TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("%d : (link,type,flags,id,length,filename)=(0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,%s)\n",(i+1),ro_j4fs_header[i].link, ro_j4fs_header[i].type, ro_j4fs_header[i].flags, ro_j4fs_header[i].id, ro_j4fs_header[i].length, ro_j4fs_header[i].filename));
	}
...
int fsd_initialize_transaction()
{
	...
	for(i=device_info.j4fs_end+1; i<device_info.j4fs_device_end; i+=J4FS_TRANSACTION_SIZE)
	{
		...
		if (error(ret)) {
			TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error(ret=0x%08x)\n",__FUNCTION__,__LINE__,ret));
	   		goto error1;
		}
...
int fsd_panic()
{
	...
	ret = FlashDevRead(&device_info, 0, J4FS_BASIC_UNIT_SIZE, buf);
	if (error(ret)) {
		TJ4FS(J4FS_TRACE_ALWAYS,("%s %d: Error\n",__FUNCTION__,__LINE__));
   		goto error1;
	}
...
int fsd_print_meta_data()
{
	...
	mst=(j4fs_mst *)buf;
	TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("(magic,from,to,end,offset_number,status,rw_start)=(0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,0x%08x,0x%08x)\n",
		mst->magic,mst->from,mst->to,mst->end,mst->offset_number,mst->status,mst->rw_start));

	if(mst->magic==J4FS_MAGIC && mst->offset_number)
	{
		TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("offset: "));
		for(i=0;i<mst->offset_number;i++)
			TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("0x%x ", mst->offset[i]));
		TJ4FS(J4FS_TRACE_FSD_PRINT_META_DATA,("\n"));
	}
